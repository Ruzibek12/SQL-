...
SELECT 
COALESCE(FirstName, LastName) AS Name FROM Employees; 
...
SELECT TOP (5) ProductID,SaleID,CustomerID
FROM Sales
...
SELECT 0.1 * AVG(Price) AS TenPercentOfAverage 
FROM Products_Discounted; 
...
SELECT * FROM Customers WHERE Email LIKE '%@gmail.com%' ESCAPE '\';
...
SELECT * FROM Employees
WHERE SALARY > ALL (SELECT SALARY FROM Employees WHERE DepartmentName = 'Salary'); 
